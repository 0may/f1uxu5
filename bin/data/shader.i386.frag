...
//
//	if (stFbo.x >= 0 && stFbo.x < resolution.x && stFbo.y >= 0 && stFbo.y < resolution.y) {
//		cFbo = vec4( texture(texFbo, stFbo).rgb * alpha, 1.0);
//	}

    vec4 cFbo = vec4( texture(texFbo, vec2((gl_FragCoord.x - offset.x), (gl_FragCoord.y - offset.y))).rgb * alpha, 1.0);
